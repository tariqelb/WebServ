...
/*   Updated: 2023/07/20 12:53:56 by hasabir          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	executeCgi(struct client &clt, std::string &filePath)
{
	...
	// {
		...
		if (!clt.cgi.pid)
		{
			...
			{
				std::fstream in("in");
				in << clt.map_request["QUERY_STRING"];
				in.close();
...
int cgi(struct webserv &web, struct client &clt)
{
	...
	try {
			executeCgi(clt, clt.map_request["URI"]);
	}
