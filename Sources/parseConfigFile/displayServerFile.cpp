...
void	displayServerFile(std::vector<struct serverfile> conf)
{
	...
	i = 0;
	while ((unsigned long)i < conf.size())
	{
		std::cout << "************************************************************" << std::endl;
		std::cout << "Host : " << conf[i].host << std::endl;
		{
			int j = 0;
			while ((unsigned long)j < conf[i].listen.size())
			{
		...
		{
			int j = 0;
			while ((unsigned long)j < conf[i].server_name.size())
			{
		...
		{
			int j = 0;
			while ((unsigned long)j < conf[i].error_page.size())
			{
		...
		{
			int j = 0;
			while ((unsigned long)j < conf[i].location.size())
			{
				std::cout << "-------------------" << "\n" << "Location : " << std::endl;
...
//				std::cout << "cgi_path : " << conf[i].location[j].cgi_path << std::endl;
				...
				int k = 0;
				while ((unsigned long)k < conf[i].location[j].error_page.size())
				{
				...
				k = 0;
				while ((unsigned long)k < conf[i].location[j].cgi.size())
				{
				...
				k = 0;
				while ((unsigned long)k < conf[i].location[j].allow.size())
				{
